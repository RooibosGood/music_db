...
| :--- | :--- | :--- |
| `play_search` | `query` (文字列) | 1. 日本語ジャンル名（例: 「ジャズ」→「Jazz」）の正規化<br/>2. MPDライブラリ検索（genre ➔ any ➔ 単語分割検索）<br/>3. キューをクリアし、検索結果の上位15曲を追加して再生開始<br/>4. 1曲目のDB解説文（`description`）を取得 |
| `play` | - | 再生を再開 (`client.play()`) |
| `pause` | - | 一時停止 (`client.pause(1)`) |
| `stop` | - | 停止 (`client.stop()`) |
| `next` | - | 次のトラックへスキップ (`client.next()`) |
| `previous` | - | 前のトラックへスキップ (`client.previous()`) |
