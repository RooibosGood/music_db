...
moOde / MPD で ReplayGain タグのゲイン値が正確に読み込まれてオーディオ出力に反映されるまでの時間を十分に確保するため、`voice_bot` では「**先に曲を moOde キューにセット（停止・待機状態） ➔ 曲紹介アナウンス（TTS発話: 約5〜15秒） ➔ 発話完了直後に音楽再生開始**」という厳格なシーケンス制御を採用しています。これにより、曲紹介トークの間に moOde 側での ReplayGain 解析・適用時間が確実に担保され、音量が急激に変わることなく適切な音量で音楽がスタートします。
（※音声発話なしの場合でも、最低3秒間の待機時間 `PLAY_DELAY_SEC = 3.0` を確保した後に再生が開始されます。）
