...
| **主な特徴** | タグ情報に加え、ローカルLLMによって生成された「楽曲区分」「気分/ムード」「エネルギーレベル」「日本語解説文」「英語解説文」を格納 |
...
| `rating` | `INTEGER` | YES | `NULL` | `1 <= rating <= 5` | **楽曲のユーザー評価（★1〜5、未評価時は `NULL`）** |
...
### 3.5 `rating`（楽曲評価・レーティング）
ユーザーによる楽曲の評価（★1〜5）を格納します。Web UI（グッド👍 / バッド👎ボタン、星アイコン直接クリック）や音声操作（「この曲いいね」「バッド」等）によって動的に更新・永続化されます。
- **未評価（無印 / `NULL`）**: 初期状態。
- **評価ルール**:
  - **未評価時 ＋ グッド👍** ➔ **★３ (`rating = 3`)**
  - **未評価時 ＋ バッド👎** ➔ **★２ (`rating = 2`)**
...
### 3.7 `title_en` / `artist_en`（英語曲名・英語アーティスト名）
...
## 5. データベースメンテナンス・修復 (`cleanup_long_tracks.py`)

データベース内のメタデータ整合性維持とクリーンアップのためのツール仕様です。

| 機能 | コマンドオプション | 概要 |
| :--- | :--- | :--- |
| **全件一括クリーンアップ** | `--all` | 長尺音源削除・年号修復・英語解説文修復を順次実行 |
| **年号の異常修復** | `--fix-years` | 5桁（`11967`等）、3桁（`160s`等）、スペース混入（`1 960s`等）を4桁西暦に正規化 |
| **英語解説文の修復** | `--fix-en-descriptions` | `description_en` 内の日本語（漢字/かな）をアルファベット/ローマ字表記に変換し、純粋な英文（DJ・TTS読み上げ用）に整形 |
| **長尺音源の削除** | `--cleanup-duration` | 20分以上の長尺トラックを検出してデータベースから除外 |
| **ドライラン (確認モード)** | `--dry-run` | データベースを書き換えずに変更対象と修正内容をプレビュー表示 |
| **確認スキップ** | `-y`, `--yes` | 確認プロンプトを表示せず即座にDBへ適用 |
