...
| :--- | :--- | :--- |
| `play_search` | `query` (文字列) | 1. **`music_meta.db` から合致する楽曲リストを完全ランダム抽出**<br/>2. 抽出された楽曲のファイル名・タイトルで MPD 内を検索し、キューに追加<br/>3. **`command_list` で即座にポーズ状態 (`play 0` + `pause 1`) を投入してデコーダを先行起動（ReplayGainタグ解析開始）**<br/>4. 1曲目のDB解説文を取得<br/>5. **音声案内完了後、または短縮ディレイ後に `pause 0` で安全に再生開始** |
| `play` | - | 再生を安全に開始・再開 (`mpd_client.safe_start_playback()`) |
| `pause` | - | 一時停止 (`client.pause(1)`) |
| `stop` | - | 停止 (`client.stop()`) |
| `next` | - | 次のトラックへスキップし、**次の曲の解説文を発話・ReplayGain事前ポーズ反映の後に再生** |
| `previous` | - | 前のトラックへスキップし、**前の曲の解説文を発話・ReplayGain事前ポーズ反映の後に再生** |
...
#### ReplayGain 曲頭バースト防止（Pattern 2: ポーズ投入 ＋ 短縮ディレイ解除方式）(`config.PRE_DECODE_DELAY_SEC`)
NFS経由での再生時、MPDがファイルヘッダからReplayGainメタデータを解析し終える前にPCM出力が先行してしまうレースコンディション（曲頭の0 dB大音量飛び出し）を防ぐため、以下の安全な再生シーケンスを採用しています。
1. キューのクリアおよび楽曲の追加（`clear` ➔ `add`）。
2. 即座に `command_list` で `play(0)` と `pause(1)` をアトミックに投入し、DAC出力を保留したままデコーダおよびヘッダ解析スレッドのみを先行起動。
3. タグ解析完了のための最小マージンとして、短時間待機（`config.PRE_DECODE_DELAY_SEC = 0.35` 秒、設定で微調整可能）。
4. 待機経過後（または曲紹介アナウンス完了後）にポーズを解除（`pause 0`）してDACへのPCM出力を開始。
