...
- **ハイブリッドUI（音声 ＋ Web Chat ＋ 設定画面）**:
  ...
  - **モバイル・レスポンシブ最適化**: スマートフォン（iPhone / Android 等）の画面幅でも「⚡ 電源操作（再起動/シャットダウン）」や「⚙️ システム設定」ボタンが画面上部ヘッダーに確実に常時表示され、快適にタップ操作可能。ステータス表示はモバイル幅でスマートなLEDドットインジケーターに自動圧縮。
  - **Settings（設定）モーダル**: デモモードのON/OFF、言語モード切替、デイリー情報ON/OFF、moOde接続先変更をWeb上から即時設定・永続化保存。
  - **Jetson 電源操作**: ヘッダーの電源ボタンから、Jetson Orin Nano Super 本体の安全な再起動・シャットダウンを即座に実行可能。誤操作防止の確認モーダルと進行オーバーレイを完備。
...
| :--- | :--- | :--- |
| `play_search` | `query` (文字列) | 1. **`music_meta.db` から合致する楽曲リストを完全ランダム抽出**<br/>2. 抽出された楽曲のファイル名・タイトルで MPD 内を検索し、キューに追加<br/>3. 1曲目のDB解説文を取得<br/>4. **曲選択後、moOde側のReplayGainタグ反映待機（3秒）または音声案内完了後に音楽再生を開始** |
| `play` | - | 再生を再開 (`client.play()`) |
| `pause` | - | 一時停止 (`client.pause(1)`) |
| `stop` | - | 停止 (`client.stop()`) |
| `next` | - | 次のトラックへスキップし、**次の曲の解説文を発話・ReplayGain反映待ち（3秒）の後に再生** |
| `previous` | - | 前のトラックへスキップし、**前の曲の解説文を発話・ReplayGain反映待ち（3秒）の後に再生** |
...
#### ReplayGain 反映待機（3秒遅延再生）仕様 (`config.PLAY_DELAY_SEC`)
moOde / MPD プレイヤーにおいて、音源ファイルの ReplayGain タグ（音量均一化ゲイン）が読み込まれて適用されるまでの時間を確保するため、曲の選択（キュー投入・選曲・曲送り等）から **3秒間の待機時間 (`config.PLAY_DELAY_SEC = 3.0`)** を設けてから音楽再生（Play）を開始します。
- **音声案内なし（Webチャット等）**: 選曲直後に画面ステータスに `⏳ ReplayGain 適用待機中 (3秒)...` を表示し、3秒待機後に再生開始。
- **音声案内あり**: アナウンス発話終了後、選曲時刻から 3秒未満であれば残りの時間を待機してから再生開始。
...
### 5.8 楽曲評価・レーティング機能 (`db.py`, `tagger.py`, `mpd_client.py`, `api.py`, `llm.py`)

再生中またはライブラリ内の楽曲に対して、**グッド👍 / バッド👎 ボタン** または **星評価アイコン（★1〜5）の直接選択**、あるいは **音声/チャット操作**（「この曲いいね」「グッド」「バッド」等）により、楽曲の評価（★ rating）を `music_meta.db` および **音源ファイル本体（FLAC/MP3/M4A等）のメタデータタグ** に直接永続化し、画面および音声フィードバックへ即座に反映します。
...
音源ファイルに付与されている ReplayGain（音量均一化タグ）を moOde / MPD 側が確実に読み込み・反映する時間を確保するため、「**先に曲をセット ➔ 曲紹介アナウンス ➔ 発話完了後に音楽再生を開始**」という厳格なシーケンス制御を採用しています。

#### 1. 処理フローシーケンス
1. **ステップ 1: 先に曲を moOde キューにセット (`control_moode`)**
   - `client.stop()` で現在の再生を即座に停止。
   - `client.clear()` でキューを初期化し、選曲された楽曲（15曲）をキューに追加。
   - `client.pause(1)` または停止状態を維持（**この時点では音楽は絶対に再生しない**）。
   - moOde / MPD はこの待機時間中にファイルの ReplayGain タグを解析し、音量調整の準備を完了します。
2. **ステップ 2: 曲紹介アナウンス（TTS音声合成・発話）**
   - 音楽が完全に停止している状態で、選曲全体の雰囲気コメントおよび1曲目の解説トーク（VOICEVOX / edge-tts）を同期的に発話。
   - 発話時間（約5〜15秒）により、ReplayGain の適用時間が確実に担保されます。
3. **ステップ 3: アナウンス発話完了直後に音楽再生を開始 (`mpd_cli.play()`)**
   - 曲紹介が完全に鳴り終わった直後に `mpd_cli.play()` を呼び出して音楽スタート。
   - ※音声発話なし（Webチャットで `speak=false` 等）の場合でも、`config.PLAY_DELAY_SEC`（デフォルト: 3.0秒）待機した後に再生を開始します。
